    ...
    public static class AddTweetsTask extends AsyncTask<NormalTweet, Void, Void> {
        ...
        protected Void doInBackground(NormalTweet... tweets) {
            ...
            for (NormalTweet tweet : tweets) {
                Index index = new Index.Builder(tweet).index("testing").type("tweet").build(); //create index and put the values in elasapicsearch, then exe through list. client through indexx

                try {
                    // where is the client?
                    DocumentResult result = client.execute(index);
                    if(result.isSucceeded()){ //if good se tweet id to result id
        ...
        protected ArrayList<NormalTweet> doInBackground(String... search_parameters) {
            ...
            try {
                ...
                if (result.isSucceeded()){
                    ...
                    for(NormalTweet onet: foundTweets){ //add to change to only display 10
                        tweets.add(onet);
                    }
//                    tweets.addAll(foundTweets);
                }else{
                    Log.i("ERROR", "search Quary failed to find anything");
                }
            }
            catch (Exception e) {
